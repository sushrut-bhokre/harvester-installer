...
// ConvertToCOS converts ZEUS config to cOS configuration.
...
// Standalone dashboards (only if rancher-monitoring is disabled)
	if a, ok := config.Addons["rancher-monitoring"]; !ok || !a.Enabled {
		templBytes, err := templFS.ReadFile(filepath.Join(templateFolder, "rancherd-13-standalone-dashboards.yaml"))
		if err == nil {
			bootstrapConfs["13-standalone-dashboards.yaml"] = string(templBytes)
		}
	}
