...
func addDiskPanel(c *Console) error {
	...
	diskFatalV.PreShow = func() error {
		diskFatalV.SetContent("No disk detected. ZEUS requires at least one disk.")
		return setPageTitle()
	}
	...
	wipeDisksTitlePanelV.SetContent("Additional ZEUS installations detected")
...
func addAskCreatePanel(c *Console) error {
	askOptionsFunc := func() ([]widgets.Option, error) {
		options := []widgets.Option{
			{
				...
				Text:  "Create a new ZEUS cluster",
			}, {
				Value: config.ModeJoin,
				Text:  "Join an existing ZEUS cluster",
			},
		}
		...
		if !alreadyInstalled {
			options = append(options, widgets.Option{
				...
				Text:  "Install ZEUS binaries only",
			})
		}
		return options, nil
	}
	...
	askCreateV.PreShow = func() error {
		...
		if alreadyInstalled {
			return c.setContentByName(titlePanel, "ZEUS already installed. Choose configuration mode")
		}
...
func addHostnamePanel(c *Console) error {
	...
	hostnameV.PreShow = func() error {
		...
		// not yet set in the ZEUS config, but we might have been given
...
func addCloudInitPanel(c *Console) error {
	...
	cloudInitV.PreShow = func() error {
		...
		return c.setContentByName(titlePanel, "Optional: remote ZEUS config")
	}
	gotoNextPage := func() error {
		...
		if !c.config.Install.Silent {
			if alreadyInstalled {
				confirmV.SetContent(options +
					"\nZEUS is already installed. It will be configured with the above configuration. Continue?\n")
			} else if installModeOnly {
				confirmV.SetContent(options +
					"\nZEUS will be copied to local disk. No configuration will be performed. Continue?\n")
			} else {
				confirmV.SetContent(options +
					"\nYour disk will be formatted and ZEUS will be installed with the above configuration. Continue?\n")
			}
		}
		c.Gui.Cursor = false
...
func addConfirmUpgradePanel(c *Console) error {
	...
	confirmV.PreShow = func() error {
		return c.setContentByName(titlePanel, fmt.Sprintf("Confirm upgrading ZEUS to %s?", version.Version))
	}
	confirmV.KeyBindings = map[gocui.Key]func(*gocui.Gui, *gocui.View) error{
		gocui.KeyEnter: func(_ *gocui.Gui, _ *gocui.View) error {
			confirmed, err := confirmV.GetData()
...
func addInstallPanel(c *Console) error {
			...
			if !alreadyInstalled {
				...
				if len(preflightWarnings) > 0 {
					if c.config.SkipChecks || preflightAck {
						...
						logrus.Info("ZEUS installation will proceed (kernel cmdline param: harvester.install.skipchecks = true)")
					} else {
	...
	installV.Title = " Installing ZEUS "
