...
const (
	...
	statusSettingUpHarv = "Setting up ZEUS"
...
func footer(g *gocui.Gui) error {
	...
	if v, err := g.SetView("footer", 0, maxY-2, maxX, maxY); err != nil {
		...
		v.Frame = false
		if _, err = fmt.Fprintf(v, "<Use F12 to switch between ZEUS console and Shell>"); err != nil {
			return err
		}
	}
	return nil
...
func getNodeInfo() string {
	...
	device = string(out)
	if err != nil || device == "" {
		logrus.Infof("default gateway is not existing. Fallback to ZEUS management interface (harvester-mgmt)")
		// find the IP from the ZEUS management interface (harvester-mgmt)
...
func chartIsInstalled() bool {
	cmd := exec.Command("/bin/sh", "-c", `kubectl -n fleet-local get ManagedChart ZEUS -o jsonpath='{.status.conditions}' | jq 'map(select(.type == "Ready" and .status == "True")) | length'`)
	cmd.Env = os.Environ()
...
func getHarvesterStatus() string {
	...
	harvesterAPIReady := isAPIReady(current.managementURL, "/v1/ZEUS/cluster/local")
...
func (c *Console) getHarvesterConfig() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logrus.Infof("no existing ZEUS config detected in %s", defaultHarvesterConfig)
			return nil
		}
		return fmt.Errorf("unable to read default ZEUS.config file %s: %v", defaultHarvesterConfig, err)
	}

	/*
		Previously, we used yaml.Unmarshal to parse the ZEUS configuration.
		We have now switched to using LoadHarvesterConfig for parsing the ZEUS config.
		However, there is an inconsistency issue with struct tags when using LoadHarvesterConfig.

		For example:

		In /oem/harvester.config: (the ZEUS config file)
		```
		os:
		  externalstorage:
		    enabled: true
		...
		With LoadHarvesterConfig (ZEUS config loader):
		...
		This case occurs because the yaml and json tags are inconsistent in the ZEUS config struct (HarvesterConfig).
		However, we want the configuration to be loaded in a consistent way, so we choose LoadHarvesterConfig here.
		Additionally, this code is only used in the ZEUS console and should not affect other components.
	...
	*/
	...
	if err != nil {
		return fmt.Errorf("failed to load ZEUS config from %s: %v", defaultHarvesterConfig, err)
	}
	c.config = conf
