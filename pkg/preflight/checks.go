...
func (c CPUCheck) Run() (msg string, err error) {
	...
	if nproc < MinCPUTest {
		msg = fmt.Sprintf("Only %d CPU cores detected. ZEUS requires at least %d cores for testing and %d for production use.",
			nproc, MinCPUTest, MinCPUProd)
	} else if nproc < MinCPUProd {
		msg = fmt.Sprintf("%d CPU cores detected. ZEUS requires at least %d cores for production use.",
			nproc, MinCPUProd)
	}
	return
}
...
func (c MemoryCheck) Run() (string, error) {
	...
	if float32(memTotalGiB) < (MinMemoryTest * wiggleRoom) {
		return fmt.Sprintf("Only %s RAM detected. ZEUS requires at least %dGiB for testing and %dGiB for production use.",
			memReported, MinMemoryTest, MinMemoryProd), nil
	} else if float32(memTotalGiB) < (MinMemoryProd * wiggleRoom) {
		return fmt.Sprintf("%s RAM detected. ZEUS requires at least %dGiB for production use.",
...
func (c KVMHostCheck) Run() (msg string, err error) {
	if _, err = os.Stat(devKvm); errors.Is(err, fs.ErrNotExist) {
		msg = "ZEUS requires hardware-assisted virtualization, but /dev/kvm does not exist."
		err = nil
	}
	return
}
...
func (c NetworkSpeedCheck) Run() (msg string, err error) {
	...
	if speedGbps < MinNetworkGbpsTest {
		...
		msg = fmt.Sprintf("Link speed of %s is only %dMpbs. ZEUS requires at least %dGbps for testing and %dGbps for production use.",
			c.Dev, speedMbps, MinNetworkGbpsTest, MinNetworkGbpsProd)
	} else if speedGbps < MinNetworkGbpsProd {
		msg = fmt.Sprintf("Link speed of %s is %gGbps. ZEUS requires at least %dGbps for production use.",
